...
class Trainer(BaseTrainer):
    ...
    def eval_step(self, data):
        ...
        occ_points_points = data.get('occ_points.points').to(device)
        occ = data.get('occ_points.occ').to(device)
        ...
        # Compute iou
        with torch.no_grad():
            p_out = self.model(iou_occ_points_points, normal_points_points, 
    ...
    def compute_loss(self, total_points):
        ...
        if self.model.decoder.out_bool:
            ...
            input_points = total_points.get('input_points', torch.empty(gt_output_bool_query_points.size(0), 0)).to(device) # 好气哦，他这里好像没有用到法向量
